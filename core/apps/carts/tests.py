...
class TestOrders(TestCase):
    ...
    def setUp(self):
        ...
        self.product = Products.objects.create(
            name='Test Product', image='1', price=100, discount=10, quantity=10, category_id=1, slug='test-product')
    ...
    def test_change_quantity_anonymous_user(self):
        cart = Cart.objects.create(user=self.test_user1, product=self.product, quantity=1)
        response = self.client.post(reverse('carts:cart_change'), {'cart_id': cart.id, 'quantity': 5})
        self.assertEqual(response.status_code, 200)
    ...
    def test_change_quantity_authenticated_user(self):
        ...
        response = self.client.post(reverse('carts:cart_change'), {'cart_id': cart.id, 'quantity': 3})
        self.assertEqual(response.status_code, 200)
        cart.refresh_from_db()
    ...
    def test_cart_remove_anonymous_user(self):
        ...
        response = self.client.post(reverse('carts:cart_remove'), {'product_id': self.product.id})
        self.assertEqual(response.status_code, 200)
